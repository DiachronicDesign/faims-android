...
public class UIRenderer implements IRestoreActionListener{
    	...
    	for (int i = 0; i < groups; i++) {
	    	...
	    	if (element instanceof GroupDef) {
	    		...
	    		if("style".equals(tabGroupName)){
		    		...
		    		parseTabGroups(directory, groupIndex, tabGroupElement,
							tabGroupCaption, tabGroupName);
	    		}
	    	}
	...
	private void parseTabGroups(String directory, FormIndex groupIndex,
			GroupDef tabGroupElement, FormEntryCaption tabGroupCaption,
			String tabGroupName) {
	...
	private void parseTab(String directory, String tabGroupName,
		IFormElement element, TabGroup tabGroup, FormIndex tabIndex) {
	...
	private void parseContainer(LinearLayout containerLayout, String directory, String tabGroupName,
			String tabName, IFormElement element, TabGroup tabGroup, FormIndex childIndex,
			Tab tab, int depth) {
			if(depth > 5){
	...
	private void parseInput(String directory, String tabGroupName,
			String tabName, IFormElement element, TabGroup tabGroup, Tab tab, FormIndex childIndex,
		...
		View view = tab.addInput(containerLayout,FormAttribute.parseFromInput(input),
				tabGroupName + "/" + tabName + "/" + viewName,viewName, directory, tabGroup.isArchEnt(), tabGroup.isRelationship(),getStyleMappings(style));
	...
	private void parseStyle(GroupDef tabGroupElement, FormIndex groupIndex) {
		FormIndex tabIndex = this.fem.getModel().incrementIndex(groupIndex,
				true);
